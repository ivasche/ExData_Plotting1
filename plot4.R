url<-"https://d396qusza40orc.cloudfront.net/exdata%2Fdata%2Fhousehold_power_consumption.zip"
download.file(url,destfile="power.zip")
unzip("power.zip")
data<-read.table("household_power_consumption.txt",sep=";",colClasses=rep("character",9),header=TRUE,stringsAsFactors=FALSE)
data$datetime<-strptime(paste(data$Date,data$Time),"%d/%m/%Y %H:%M:%S")
suppressWarnings(data<-data.frame(data[,10],sapply(data[,3:9],as.numeric)))
names(data)<-c("Date",names(data)[2:8])
data.set<-subset(data,(data$Date>=as.POSIXlt("2007-02-01 00:00:00"))&(data$Date<as.POSIXlt("2007-02-03 00:00:00")))
remove(data)
png(file="plot4.png",bg="transparent",width=480,height=480,units="px")
par(mar=c(4.1,4.1,2.1,2.1))
par(mfrow=c(2,2))
plot(data.set$Date,data.set$Global_active_power,type="l",xlab="",ylab="Global Active Power",bg="transparent")
plot(data.set$Date,data.set$Voltage,type="l",xlab="datetime",ylab="Voltage",bg="transparent")
plot(data.set$Date,data.set$Sub_metering_1,type="l",xlab="",ylab="Energy sub metering",bg="transparent")
lines(data.set$Date,data.set$Sub_metering_2,col="red")
lines(data.set$Date,data.set$Sub_metering_3,col="blue")
legend("topright",names(data.set)[6:8],lty=1,col=c("black","red","blue"))
plot(data.set$Date,data.set$Global_reactive_power,type="l",xlab="datetime",ylab="Global_active_power",bg="transparent")
dev.off()